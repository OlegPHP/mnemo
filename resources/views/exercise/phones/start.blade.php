<x-layouts.app :title="'Мнемотехники — номера телефонов'">
    <div class="flex flex-col items-center justify-center min-h-[70vh] px-4">
        <div class="w-full max-w-md bg-neutral-100 dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-800 rounded-2xl p-8 shadow-md transition-colors">
            ...
            <form action="{{ route('exercises.phones.learn', ['exercise' => $exercise->slug]) }}" method="GET" class="space-y-6">
                <flux:select name="number" placeholder="Количество номеров" class="w-full cursor-pointer">
                    @for ($i = 1; $i <= 10; $i++)
